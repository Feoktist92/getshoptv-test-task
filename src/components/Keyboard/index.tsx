...
const Keyboard: React.FC<KeyboardProps> = ({
    ...
    async function validateRussianPhoneNumber(phoneNumber: string): Promise<void> {
        try {
            const url = `https://api.apilayer.com/number_verification/validate?number=${phoneNumber}`;
            const headers = {
                apikey: 'tpLQvbn6rZu5O4Aenow8FbT4ySidEmeO',
            };
        ...
        const handleKeyboardEvent = (event: KeyboardEvent) => {
            ...
            switch (key) {
                ...
                case 'Enter': {
                    ...
                    const focusedElement = elementsRef.current[focusedElementIndex];
                    if (focusedElement) {
                ...
                case 'ArrowLeft': {
                    setIsInactive(false);
                    setFocusedElementIndex((prevIndex) =>
                        prevIndex > 0 ? prevIndex - 1 : elementsRef.current.length - 1
                    );
                    break;
                }
                ...
                case 'ArrowRight': {
                    setIsInactive(false);
                    setFocusedElementIndex((prevIndex) =>
                        prevIndex < elementsRef.current.length - 1 ? prevIndex + 1 : 0
                    );
                    break;
                }
            ...
            {isPhoneNumberValid ? (
                <label className={styles.label}>
                    ...
                    <span
                        ...
                        ref={(el) => (elementsRef.current[11] = el as HTMLButtonElement | null)}
                    >
